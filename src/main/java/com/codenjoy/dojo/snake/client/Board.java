...
public class Board extends AbstractBoard<Elements> {
    ...
    public boolean isEmptyPlaceOrApple(Point from, Direction direction, Board board) {
        char[][] field = board.getField();
        Point point = direction.change(from);
        return  (field[point.getX()][point.getY()] == Elements.NONE.ch() ||
                field[point.getX()][point.getY()] == Elements.GOOD_APPLE.ch());
    }

    public boolean isTailOn(Point from, Direction direction) {
        List<Point> snake = getSnake();
        Point point = direction.change(from); // походу метод для выявления пустых и нет клеток
